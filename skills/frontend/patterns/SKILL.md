...
The skill carries the put.io-wide code defaults — type/schema-driven boundaries, parse-don't-validate, impossible-states-impossible, state machines for bug-sensitive flows. The `.patterns/` folder is the per-repo extension: where a repository pins its own concrete choices (which schema lib, which state-machine lib, which styling stack) so future contributors and agents do not re-litigate them on every PR.
...
3. If `.patterns/` is silent on the area you are touching, read [references/frontend-defaults.md](references/frontend-defaults.md) and apply the put.io default unless the repo signals otherwise.
